...
<div class="container">
    <h3 class="mb-4">{{ __('customer rate info details') }}</h3>
    <div class="card">
    <form action="{{route('Admin.medical-device-info.update')}}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
        {{-- <div class="col-12">
            <div class="card mb-6">
        ...
        @foreach (config('translatable.locales') as $locale)
            ...
            <label>{{ __('system.'.$locale.'.secondary_title') }}</label>
            <input class="form-control" name="{{$locale}}[secondary_title]"   value="{{ isset($data) ? $data->translateOrNew($locale)->secondary_title : old($locale . '.secondary_title')  }}" type="text" required>

            <label>{{ __('system.'.$locale.'.secondary_sub_title') }}</label>
            <input class="form-control" name="{{$locale}}[secondary_sub_title]"   value="{{ isset($data) ? $data->translateOrNew($locale)->secondary_sub_title : old($locale . '.secondary_sub_title')  }}" type="text" required>

            <label>{{ __('system.'.$locale.'.secondary_description') }}</label>
            <textarea class="form-control" name="{{$locale}}[secondary_description]" rows="3"  value="" type="text" required>{{ isset($data) ? $data->translateOrNew($locale)->secondary_description : old($locale . '.secondary_description')  }} </textarea>
