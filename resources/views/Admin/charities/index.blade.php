...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="{{route('Admin.charities.index')}}">{{__('Charities Supports')}}</a></li>
  </ol>
</nav>
...
<div class="card">
  <div class="card-body">
    <div class="row">
        <div class="col-3">
            <a href="{{route('Admin.charities.create')}}" class="btn " style="background-color: #267B26 ; color:white"> {{__('system.add')}}</a>
          </div>
    </div>
  ...
  <x-edit-modal>
    ...
    <x-slot:route>
        {{route('Admin.charities.update',0)}}
    </x-slot>
    @method('put')
  ...
  <x-delete-modal>
    <x-slot:route>
        {{route('Admin.charities.destroy',0)}}
    </x-slot>
    @method('DELETE')
