			...
			<div class="flex-start align-center">
					...
					<ul class="main-menu list-unstyled flex-center">
						...
						<li class="menu-item has-children">
							...
							<ul class="sub-menu list-unstyled">
								@foreach (\App\Models\Specialtie::get() as $row)
							     	<li><a href="{{route('Site.specialtie',$row->slug)}}">{{$row->title}}</a></li>
								@endforeach
						...
						<li class="menu-item  has-children">
							<a href="#">{{__('teams')}}</a>
								<ul class="sub-menu list-unstyled">
									@foreach (\App\Models\Specialtie::get() as $row)
										<li><a href="{{route('Site.teams.index',$row->slug)}}">{{$row->title}}</a></li>
									@endforeach
								</ul>
							</li>
				...
				<div class="user-logged-in flex-start">
					<a href="{{ route('Site.cart.index') }}" class="head-icon flex-start align-center">
						<img src="{{asset('siteassets/images/bag.svg')}}" alt="Shopping cart">
						<span>{{__('cart')}}</span>
					</a>
